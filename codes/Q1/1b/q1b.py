...
for key, df in df_list.items():
    ...
    new_df=all_err_df.drop(columns=['Remarks'])
    logging.debug(f" ---> new_df= {new_df.to_string()}")
    logging.debug(f" ---> len new_df= {len(new_df)}")
    new_df.drop_duplicates(keep="first",inplace=True)
    no_dup_df=new_df
    logging.debug(f" ---> no_dup_df= {no_dup_df.to_string()}") 
    logging.debug(f" ---> len no_dup_df= {len(no_dup_df)}")

    remarks_list=[]
    for index,row in no_dup_df.iterrows(): 
        df_err=all_err_df
        for key in q1b.combination_set:
            df_err=df_err[df_err[key] == row[key]]
        
        logging.debug(f" ---> df_err= {df_err.to_string()}") 
        all_err = ','.join(df_err["Remarks"])

        logging.debug(f" ---> all_err= {all_err}") 
        remarks_list.append(all_err)

    if no_dup_df is not None:
        no_dup_df.insert(0, 'Remarks', remarks_list)

    logging.debug(f" ---> len no_dup_df= {len(no_dup_df)}")    
    logging.debug(f" ---> no_dup_df= {no_dup_df.to_string()}")         

    q1b.output_excel(filename,sheet_name,no_dup_df)
